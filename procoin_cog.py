...
class BotInterface(commands.Cog, ProCoin):
    ...
    async def purchase(self, ctx, *parameters: str) -> None:
        ...
        if not transaction_result:
            await ctx.send('Transaction failed. We need some actual ' \
                         'exceptions instead of just True or False (agreed).')

        await ctx.send(f'<@{user_id}> bought {qty}'\
                       f' {self.items.lookup(item_string)}')

    @tasks.loop(minutes=60.0)
    async def _update_store(self) -> None:
        self._generate_store()

def setup(bot):
    bot.add_cog(BotInterface())
